...
fig, axes = plt.subplots(2, 3, figsize=(30, 16), dpi=600)
axes = axes.flatten()
...
def plot_on_axis(ax, acc_across_CL_stream, method_name, dataset_name, num_splits):
    ...
    # Apply different settings based on number of splits
    if num_splits <= 6:  # For 5-splits (CIFAR100)
        ax.set_xticklabels([f'Task {i+1}' for i in stream_points], fontsize=16)
        ax.tick_params(axis='y', labelsize=16)
        ax.set_ylim(0.6, 1.0)
        ax.legend(loc='lower left', fontsize=14)
    else:  # For 10-splits (CIFAR10)
        ax.set_xticklabels([f'Task {i+1}' for i in stream_points], fontsize=12)
        ax.tick_params(axis='y', labelsize=14)
        ax.set_ylim(0.4, 1.0)
        # Create a more compact legend with smaller font size
        leg = ax.legend(loc='lower left', fontsize=8, ncol=2)
...
plt.savefig('all_methods_combined_plot.png', dpi=600)
