...
reg_weight=$7
model_dist_type=$8
# If seed is -1, run for seeds 42-46
if [ $seed -eq -1 ]; then
    for seed_value in {42..46}; do
        # TRAIN
        ...
        python my_finetune_splitted_pre_merged_dist.py \
            ...
            --model_dist_type ${model_dist_type} \
                |& tee -a ${out_dir}/splits:${n_splits}-mode:${pre_merged_mode}-ep:${epochs}-seed:${seed_value}.out
        ...
        python merge_splitted.py \
            ...
            --sequential_finetuning \
            --seed ${seed_value} \
                |& tee -a ${out_dir}/merge-${n_splits}-mode:${pre_merged_mode}-ep:${epochs}-seed:${seed_value}.out
    done
else
    # Original code for single seed
    # TRAIN
    # out_dir=outs/${model}/pre_merged_distance/class_incremental/ft/${dataset}
    # mkdir -p ${out_dir}
